...
function decodeRunBitpackedNew(cursor, count, opts) {
  if (count % 8 !== 0) {
    throw 'must be a multiple of 8';
  }
  ...
  let bitsRemaining = 0;
  let byteI = 0;
  let byteCount = bitWidth * count / 8;
  while (i < count) {
    if (bitsRemaining < bitWidth) {
      if (byteI >= byteCount) {
        break;
      }
      bits |= cursor.buffer[cursor.offset + byteI] << bitsRemaining;
      bitsRemaining += 8;
      byteI += 1;
      // bits |= cursor.buffer.readUInt32LE(cursor.offset + byteI) << bitsRemaining;
      // const addedBits = 32 - bitsRemaining;
      // bitsRemaining += addedBits;
      // byteI += Math.floor(addedBits / 8);
      continue;
    }
  ...
  //   var offsetMin = Math.ceil(bitWidth * i / 8. + bitWidth / 8. - 4);
  //   if (offsetMin < 0) {
  //       offsetMin = 0;
  //   }
  //   console.log(bitWidth, i, offsetMin);
  //   var wordWithValue = cursor.buffer.readUInt32LE(cursor.offset + offsetMin);

  //   var offsetInWord = bitWidth * i - offsetMin * 8;
  //   var leftInWord = 32 - bitWidth - offsetInWord;
  //   values[i] = (wordWithValue >> leftInWord) % mask;
  // }
...
exports.decodeValues = function(type, cursor, count, opts) {
  ...
  while (values.length < count) {
    ...
    if (header & 1) {
      res = decodeRunBitpackedNew(cursor, (header >> 1) * 8, opts);
    } else {

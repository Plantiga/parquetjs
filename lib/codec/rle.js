...
function decodeRunBitpacked(cursor, count, opts) {
  ...
  while (byteI < byteCount) {
    const endByte = Math.min(byteCount, byteI + Math.floor((32 - bitsRemaining) / 8));
    for (; byteI < endByte; byteI++, bitsRemaining += 8) {
      bits |= cursor.buffer[cursor.offset + byteI] << bitsRemaining;
    }

    while (bitsRemaining >= bitWidth) {
      values[i++] = bits & mask;
      bits >>>= bitWidth;
